...
public class GenerateDots : MonoBehaviour
{
    ...
    public static int doll_mul; // 웨이브당 완성품 인형 개수
    public static int indoll_mul; // 웨이브당 불량품 인형 개수
    ...
    void touchDot()
    {
        ...
        { // 인게임 점수 계산 추가 해야됨
            ...
            {
                notScore.notscore += indoll_mul;
                Score.score += indoll_mul;
                score += indoll_mul;
                indoll += indoll_mul;
            }
            else
            {
                dollNum.doll_amount += doll_mul;
                Score.score += doll_mul;
                score += doll_mul;
                doll += doll_mul;
                Settings.coin += (doll_mul * 5);
            }
            if (isScore <= bearRate)
            {
                Settings.bear_doll++;
            ...
            Debug.Log("완성 인형");
            Debug.Log(dollNum.doll_amount);
            Debug.Log("불량 인형");
            Debug.Log(notScore.notscore);
        }
    ...
    void gameOver() // 게임오버 
    {
        ...
        xList.Clear();
        yList.Clear();
        Settings.incomplete_doll += indoll * 3;
